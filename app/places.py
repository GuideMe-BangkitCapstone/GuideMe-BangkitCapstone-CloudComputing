...
def getUserVisitHistory(user_id):
    data = None
    data = db_read("""SELECT history_id, user_id, name, photoUrl, created_at FROM users_visit_history INNER JOIN places ON users_visit_history.place_id = places.place_id WHERE user_id = %s""", (user_id,))
